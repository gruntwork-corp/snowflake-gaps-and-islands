...
Our cutoff is 15 min (60*15 seconds), so we expect the see the return of row 1 and 3
*/
...
select
    cte.*
from cte, table(UDTF_PERFORMANCE_TESTING.SRC.PARTITION_CLEANER(cte.ID, cte.START_TIME, cte.END_TIME, 60*15) over (partition by cte.BIKE_ID order by cte.ID asc));