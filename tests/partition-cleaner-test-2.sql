...
Our cutoff is 50 seconds, so we expect the see the return of all rows (1, 2 and 3)
*/
...
select
    cte.*
from cte, table(UDTF_PERFORMANCE_TESTING.SRC.PARTITION_CLEANER(cte.ID, cte.START_TIME, cte.END_TIME, 50) over (partition by cte.BIKE_ID order by cte.ID asc));